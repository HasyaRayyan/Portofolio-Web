...
export default function Contact() {
  ...
  const submit = async (e) => {
    ...
    if (!form.name || !form.email || !form.message) {
      setStatus({ loading: false, ok: false, err: 'Mohon isi nama, email, dan pesan Anda.' });
      return;
    }
    ...
    try {
      const res = await fetch(`https://formsubmit.co/ajax/${CONTACT_EMAIL}`, {
        ...
        body: JSON.stringify({
          name: form.name,
          email: form.email,
          _subject: form.subject || `Pesan Portofolio Baru dari ${form.name}`,
      ...
      if (res.ok) {
        ...
        setTimeout(() => setStatus((s) => ({ ...s, ok: false })), 7000);
      } else {
        throw new Error('API error');
      }
    } catch (err) {
      // Fallback ke mailto jika fetch diblokir oleh ekstensi adblock/browser
      ...
      <div className="container">
        <div className="reveal" style={{ textAlign: 'center', marginBottom: '64px' }}>
          ...
          <p className="section-sub" style={{ marginBottom: 0 }}>
            Punya ide produk, tawaran magang/kerja, atau ingin kolaborasi membangun aplikasi? Hubungi saya langsung!
          </p>
        </div>
        ...
        <div className="contact-grid">
          ...
          <div className="contact-left reveal from-left">
            ...
            <p>
              Saya selalu terbuka untuk mendiskusikan pengembangan aplikasi baru, kolaborasi rekayasa software,
              atau sekadar bertukar wawasan seputar teknologi. Kirim pesan dan pesan Anda akan langsung terkirim ke kotak masuk saya.
            </p>
            ...
            <form className="contact-form" onSubmit={submit}>
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input id="contact-name" name="name" type="text" className="form-input" placeholder="Nama Anda" value={form.name} onChange={change} disabled={status.loading} required />
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input id="contact-subject" name="subject" type="text" className="form-input" placeholder="Topik diskusi atau kolaborasi" value={form.subject} onChange={change} disabled={status.loading} />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="contact-message">Pesan</label>
                <textarea id="contact-message" name="message" className="form-input" placeholder="Tuliskan detail ide atau pesan Anda di sini..." value={form.message} onChange={change} disabled={status.loading} required />
              </div>
              <button type="submit" className="btn btn-primary" style={{ width: '100%' }} disabled={status.loading}>
                {status.loading ? 'Mengirim pesan langsung...' : 'Kirim Pesan Sekarang'}
                {!status.loading && (
              ...
              {status.ok && (
                <div className="submit-msg success">
                  ✓ Pesan berhasil dikirim ke hasyarayyanbm@gmail.com! Terima kasih, saya akan membalas secepatnya.
                </div>
              )}
            </form>
          </div>
